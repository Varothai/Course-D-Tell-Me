...
export default function HomeClient({ initialReviews = [] }: { initialReviews?: ReviewWithUserInteraction[] }) {
  ...
  const [reviews, setReviews] = useState<ReviewWithUserInteraction[]>(initialReviews ?? [])
  ...
  // Fetch reviews when component mounts (use server-passed initialReviews so they show right away)
  useEffect(() => {
    if (initialReviews?.length && !reviewsCache.current) {
      reviewsCache.current = initialReviews
    }
    fetchReviews()
